...
//密匙 key
#define gkey            @"sjdqmp20161205#_316@gfmt"
//偏移量
#define gIv             @"01234567789"
...
- (NSString *)doDecEncryptStr:(NSString *)encryptStr {
    ...
    CCCryptorStatus ccStatus = CCCrypt(kCCDecrypt,//  解密
                                       kCCAlgorithm3DES,//  解密根据哪个标准（des，3des，aes。。。。）
...
- (NSDictionary *)doDecEncryptAndConvertToDict:(NSString *)convertStr{

    NSString *decEncryptStr = [self doDecEncryptStr:convertStr];
//    NSLog(@"%@",decEncryptStr);
    NSData *jsonData = [decEncryptStr dataUsingEncoding:NSUTF8StringEncoding];
    if (!jsonData) {
        return @{};
    }
...
+ (NSDictionary *)doDecEncryptAndConvertToDict:(NSString *)convertStr{
    ...
    NSString *decEncryptStr = [[[JKEncrypt alloc]init] doDecEncryptStr:convertStr];
    //    NSLog(@"%@",decEncryptStr);
    NSData *jsonData = [decEncryptStr dataUsingEncoding:NSUTF8StringEncoding];
    NSDictionary *dataDict = [NSJSONSerialization JSONObjectWithData:jsonData options:NSJSONReadingMutableContainers error:nil];
...
- (NSArray *)doDecEncryptAndConvertToArr:(NSString *)convertStr{
