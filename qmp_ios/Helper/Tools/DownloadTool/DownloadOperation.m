...
-(NSString*)destinationPath:(NSString*)remoteUrl{
    ...
    BOOL isDirectory;
    if (![fileMgr fileExistsAtPath:destinationPath isDirectory:&isDirectory]) {
        [fileMgr createDirectoryAtPath:destinationPath withIntermediateDirectories:YES attributes:nil error:nil];
    }
    NSString *fileDestionPath = [destinationPath stringByAppendingPathComponent:[remoteUrl fileName]];
    return fileDestionPath;
    
}
-(instancetype)initWithUrl:(NSString*)remoteUrl progress:(DownloadProgress)progress downloadFinish:(DownloadFinish)downloadFinish{
    if (self = [super init]) {
...
-(void)addUrl:(NSString*)remoteUrl progress:(DownloadProgress)progress downloadFinish:(DownloadFinish)downloadFinish{
    if (_downTask) {
        [_downTask cancel];
    }
    self.finish = NO;
...
- (void)main { //具体的下载任务
    ...
    if ([filemgr fileExistsAtPath:[self cachePath:self.url]]) {
        ...
        _dataTask = [_session dataTaskWithURL:[NSURL URLWithString:self.url]];
...
didCompleteWithError:(nullable NSError *)error{
    if ((task.state == NSURLSessionTaskStateCompleted) && (error == nil)) {
        ...
        [[DownloadTool shared] updateDownloadStatusUrl:self.url isDownload:YES];
...
-(void)URLSession:(NSURLSession *)session dataTask:(nonnull NSURLSessionDataTask *)dataTask didReceiveResponse:(nonnull NSURLResponse *)response completionHandler:(nonnull void (^)(NSURLSessionResponseDisposition))completionHandler{
    ...
    QMPLog(@"下载准备返回response-------%@",response);
//    NSHTTPURLResponse *httpResponse = (NSHTTPURLResponse*)response;
//    if (httpResponse.statusCode == 206) {  //206是下载失败了   }
    
    outputStream = [NSOutputStream outputStreamToFileAtPath:[self cachePath:self.url] append:YES];
    [outputStream open];
    if (fileTotalSize == 0) {
...
//    NSLog(@"进度值: %lf",currentFileSize * 1.0 / fileTotalSize);
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        self.progress(progress,dataTask.currentRequest.URL.absoluteString);
