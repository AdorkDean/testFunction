...
#ifndef kCFCoreFoundationVersionNuer_iOS_7_0
#define kCFCoreFoundationVersionNuer_iOS_7_0 847.20
#endif

#ifndef kCFCoreFoundationVersionNuer_iOS_8_0
#define kCFCoreFoundationVersionNuer_iOS_8_0 1129.15
#endif

#define MainThreadAssert() NSAssert([NSThread isMainThread], @"ProgressHUD needs to be accessed on the main thread.");

CGFloat const ProgressMaxOffset = 1000000.f;

static const CGFloat DefaultPadding = 4.f;
static const CGFloat DefaultLabelFontSize = 16.f;
static const CGFloat DefaultDetailsLabelFontSize = 12.f;
...
@interface ProgressHUDRoundedButton : UIButton
@end
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    ProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (ProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (ProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = ProgressHUDAnimationFade;
    ...
    _contentColor = RGBa(150, 149, 155, 1);
...
- (void)animateIn:(BOOL)animatingIn withType:(ProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == ProgressHUDAnimationZoom) {
        type = animatingIn ? ProgressHUDAnimationZoomIn : ProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == ProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == ProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == ProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == ProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
#pragma clang diagnostic pop
    ...
    if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuer_iOS_7_0) {
...
- (void)done {
    ...
    ProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<ProgressHUDDelegate>delegate = self.delegate;
...
- (void)setupViews {
    ...
    BackgroundView *backgroundView = [[BackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = ProgressHUDBackgroundStyleSolidColor;
    ...
    BackgroundView *bezelView = [BackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:DefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:DefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;
    
    UIButton *button = [ProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:DefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    if (mode == ProgressHUDModeIndeterminate) {
        ...
        [indicator removeFromSuperview];
        indicator = [[BarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[ProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[ProgressHUD class]]];
#endif
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [RoundProgressView appearanceWhenContainedIn:[ProgressHUD class], nil];
#else
        appearance = [RoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[ProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((RoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((RoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[BarProgressView class]]) {
        BarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [BarProgressView appearanceWhenContainedIn:[ProgressHUD class], nil];
#else
        appearance = [BarProgressView appearanceWhenContainedInInstancesOfClasses:@[[ProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((BarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((BarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    BackgroundView *bezelView = self.bezelView;
...
- (void)layoutSubviews {
    ...
    // would trigger a zoie object access.
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? DefaultPadding : 0.f;
...
- (void)updateForCurrentOrientationAnimated:(BOOL)animated {
    ...
    BOOL iOS8OrLater = kCFCoreFoundationVersionNuer >= kCFCoreFoundationVersionNuer_iOS_8_0;
...
- (void)drawRect:(CGRect)rect {
    ...
    BOOL isPreiOS7 = kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNuer_iOS_7_0;
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuer_iOS_7_0) {
            ...
            if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuer_iOS_8_0) {
...
- (void)setStyle:(ProgressHUDBackgroundStyle)style {
    if (style == ProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNuer_iOS_7_0) {
...
@implementation ProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [ProgressHUD allHUDsForView:view];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable ProgressHUDCompletionBlock)completion {
...
- (BOOL)diackground {
    BackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == ProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDiackground:(BOOL)diackground {
    MainThreadAssert();
    self.backgroundView.style = ProgressHUDBackgroundStyleSolidColor;
    self.backgroundView.color = diackground ? [UIColor colorWithWhite:0.f alpha:.2f] : [UIColor clearColor];
}
...
@implementation ProgressHUDRoundedButton
