...
- (id)init
{
    ...
    if (self) {
        ...
        _tagsMinPadding = 10;/** 标签最小内边距 default is 10*/
        _tagsMargin = 12;/** 标签间距 default is 10*/
        _tagsLineSpacing = 12;/** 标签行间距 default is 10*/
        _leadingMargin = 16;
    }
    return self;
}
...
- (void)setTagsArray:(NSArray *)tagsArray
{
    ...
    CGFloat btnX = _leadingMargin;/** 标签间距 default is 10*/
    CGFloat btnW = 0;
    ...
    for (NSInteger i=0; i<lastIndexs.count; i++) {
        ...
        CGFloat tagX = _leadingMargin;
        CGFloat tagY = top + _tagsLineSpacing + (_tagsLineSpacing + tagH) * i;
...
- (void)setAreasArray:(NSArray *)areasArray{
    ...
    CGFloat top = 5;
    CGFloat btnX = _tagsMargin;/** 标签间距 default is 10*/
    CGFloat btnW = 0;
    ...
    for (NSInteger i=0; i<areasArray.count; i++) {
        
        btnW = [self sizeWithText:areasArray[i] font:TagsTitleFont].width + _tagsMinPadding * 2;
        
        if (i < areasArray.count-1) {
            nextWidth = [self sizeWithText:areasArray[i+1] font:TagsTitleFont].width + _tagsMinPadding * 2;
        }
        CGFloat nextBtnX = btnX + btnW + _tagsMargin;
        // 如果下一个按钮，标签最右边则换行
        if ((nextBtnX + nextWidth) > (WIDTH - _tagsMargin)) {
    ...
    for (NSInteger i=0; i<lastIndexs.count; i++) {
        ...
        // 从数组中截取每一行的数组
        NSArray *newArr = [areasArray subarrayWithRange:NSMakeRange(location, length)];
        location = lastIndex + 1;
