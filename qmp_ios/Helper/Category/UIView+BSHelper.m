...
//    view.layer.borderColor = [UIColor whiteColor].CGColor;
    ...
    view.layer.shadowColor = H9COLOR.CGColor;
}
...
-(UIImage *)BS_screenShotImageOrigin
{
    UIGraphicsBeginImageContextWithOptions(self.frame.size, NO, 0.0);
    [self.layer renderInContext:UIGraphicsGetCurrentContext()];//ios6
    UIImage *image = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    return image;
}

-(UIImage *)BS_screenShotImageOrigin_7
{
    UIGraphicsBeginImageContextWithOptions(self.frame.size, NO, 0.0);
    //获取图像
    if ([self respondsToSelector:@selector(drawViewHierarchyInRect:afterScreenUpdates:)]) {
        [self drawViewHierarchyInRect:self.bounds afterScreenUpdates:YES];//ios7
    }else {
        [self.layer renderInContext:UIGraphicsGetCurrentContext()];//ios6
    }
    UIImage *image = UIGraphicsGetImageFromCurrentImageContext();
...
- (UIImage *)captureScrollView:(UIScrollView *)scrollView
{
    ...
    UIGraphicsBeginImageContext(scrollView.contentSize);
    {
        ...
        if ([scrollView respondsToSelector:@selector(drawViewHierarchyInRect:afterScreenUpdates:)]) {
            [scrollView drawViewHierarchyInRect:CGRectMake(0, 0, scrollView.contentSize.width, scrollView.contentSize.height) afterScreenUpdates:YES];//ios7
        }else {
            [scrollView.layer renderInContext:UIGraphicsGetCurrentContext()];//ios6
        }
...
-(UIImage*)imageFromView:(UIView *)theView atFrame:(CGRect)aRect
{
    ...
    //获取图像
        [theView.layer renderInContext:UIGraphicsGetCurrentContext()];//ios6
...
-(void)cornerRadius:(int) radius borderWidth:(CGFloat) width borderColor:(UIColor *)color
{
    CGRect frame = self.frame;
    CAShapeLayer *maskLayer = [CAShapeLayer layer];
    UIBezierPath *path = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(0, 0, frame.size.width, frame.size.height) cornerRadius:radius] ;
...
@implementation UIView (AnyCorner)
- (void)cornerRadius:(int)radius rectCorner:(UIRectCorner)rectCorner{
