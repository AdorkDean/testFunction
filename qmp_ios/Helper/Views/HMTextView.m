...
#warning 如果是copy策略，setter最好这么写
    ...
    if (placehoder.length > 20) {
        NSMutableParagraphStyle *style = [[NSMutableParagraphStyle alloc] init];
        style.lineSpacing = 4.0;
        NSAttributedString *aStr = [[NSAttributedString alloc] initWithString:placehoder attributes:@{NSFontAttributeName:self.font, NSParagraphStyleAttributeName: style}];
        self.placehoderLabel.attributedText = aStr;
    }else{
...
- (void)layoutSubviews
{
    ...
    self.placehoderLabel.top = 0;
    self.placehoderLabel.left = 4;
...
//    CGSize placehoderSize = [self.placehoder sizeWithFont:self.placehoderLabel.font constrainedToSize:maxSize];
    CGSize placehoderSize = [self.placehoder boundingRectWithSize:maxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.font, NSParagraphStyleAttributeName: style} context:nil].size;
