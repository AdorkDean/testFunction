...
#define TRIANGLE 5.0
#define NoticeColor BLUE_TITLE_COLOR
static NSMutableArray *noticeArray;

@interface HYNoticeView()

@property(nonatomic,strong)UIColor *noticeColor;
@property(nonatomic,strong)UIColor *titleColorColor;
...
-(void)drawTriangle:(CGContextRef)ctx andRect:(CGRect)rect{
    ...
    switch (_position) {
        ...
        case HYNoticeViewPositionBottomRight:
            ...
            break;
        case HYNoticeViewPositionBottomLeft:
            sPoints[0] =CGPointMake(rect.size.width*0.25, rect.size.height);//坐标1
            sPoints[1] =CGPointMake((rect.size.width*0.25-SIDELENGTH*0.5), rect.size.height-TRIANGLE);//坐标2
            sPoints[2] =CGPointMake((rect.size.width*0.25+SIDELENGTH*0.5), rect.size.height-TRIANGLE);//坐标3
            break;
...
-(void)drawRectangle:(CGContextRef)ctx andRect:(CGRect)rect{
    ...
    UIBezierPath *roundedRect = [UIBezierPath bezierPathWithRoundedRect:rectangleRect cornerRadius:4.0f];
    [self.self.noticeColor setFill];
...
-(instancetype)initWithFrame:(CGRect)frame text:(NSString*)text bgColor:(UIColor*)bgColor textColor:(UIColor*)textColor position:(HYNoticeViewPosition)position{
    
    if (self = [super initWithFrame:frame]) {
        self.backgroundColor = [UIColor clearColor];
        self.noticeColor = bgColor;
        _position = position;
        _closeClick = nil;
        _noticeClick = nil;
...
-(instancetype)initWithFrame:(CGRect)frame text:(NSString*)text position:(HYNoticeViewPosition)position closeBlock:(void(^)())closeBlock noticeBlock:(void(^)())noticeBlock{
    if (self = [super initWithFrame:frame]) {
        self.backgroundColor = [UIColor clearColor];
        self.noticeColor = BLUE_BG_COLOR;
        _position = position;
...
+(void)hideNoticeInView:(UIView*)view{
    
    for (UIView *subV in view.subviews) {
        if ([subV isKindOfClass:[HYNoticeView class]]) {
