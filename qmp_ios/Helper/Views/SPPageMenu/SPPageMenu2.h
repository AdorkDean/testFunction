...
typedef NS_ENUM(NSInteger, SPPageMenuTrackerStyle2) {
    SPPageMenuTrackerStyle2Line = 0,                  // 下划线,默认与item等宽
    SPPageMenuTrackerStyle2LineLongerThanItem,        // 下划线,比item要长(长度为item的宽+间距)
    SPPageMenuTrackerStyle2LineAttachment,            // 下划线依恋样式
    SPPageMenuTrackerStyle2TextZoom,                  // 文字缩放
    SPPageMenuTrackerStyle2RoundedRect,               // 圆角矩形
    SPPageMenuTrackerStyle2Rect                       // 矩形
};

typedef NS_ENUM(NSInteger, SPPageMenuPermutationWay2) {
    SPPageMenuPermutationWay2ScrollAdaptContent = 0,   // 自适应内容,可以左右滑动
    SPPageMenuPermutationWay2NotScrollEqualWidths,     // 等宽排列,不可以滑动,整个内容被控制在pageMenu的范围之内,等宽是根据pageMenu的总宽度对每个item均分
    SPPageMenuPermutationWay2NotScrollAdaptContent     // 自适应内容,不可以滑动,整个内容被控制在pageMenu的范围之内,这种排列方式下,自动计算item之间的间距,itemPadding属性设置无效
};

typedef NS_ENUM(NSInteger, SPItemImagePosition2) {
    SPItemImagePosition2Default,   // 默认图片在左边
    SPItemImagePosition2Left,      // 图片在左边
    SPItemImagePosition2Top,       // 图片在上面
    SPItemImagePosition2Right,     // 图片在右边
    SPItemImagePosition2Bottom     // 图片在下面
};

@class SPPageMenu2;

@protocol SPPageMenuDelegate2 <NSObject>

@optional
- (void)pageMenu:(SPPageMenu2 *)pageMenu functionButtonClicked:(UIButton *)functionButton;
// 若以下2个代理方法同时实现了，那么只会走第2个代理方法
- (void)pageMenu:(SPPageMenu2 *)pageMenu itemSelectedAtIndex:(NSInteger)index;
- (void)pageMenu:(SPPageMenu2 *)pageMenu itemSelectedFromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex;


- (void)pageMenu:(SPPageMenu2 *)pageMenu currentButtonClickAtIndex:(NSInteger)index;
@end

@interface SPPageMenu2 : UIView
...
/** item的标题字体 */
@property (nonnull, nonatomic, strong) UIFont *selectedItemTitleFont;
/** 选中的item标题颜色 */
...
/** 代理 */
@property (nonatomic, weak) id<SPPageMenuDelegate2> delegate;
 ...
 */
- (void)setTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(SPItemImagePosition2)imagePosition imageRatio:(CGFloat)ratio forItemIndex:(NSUInteger)itemIndex;
 ...
 *  @param state            控件状态
 */
- (void)setFunctionButtonTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(SPItemImagePosition2)imagePosition imageRatio:(CGFloat)ratio forState:(UIControlState)state;
...
@property (nonatomic, assign) BOOL isAttention; /// 修正关注的游标位置
