...
#import "SPPageMenu2.h"

#define tagBaseValue2 100
#define scrollViewContentOffset2 @"contentOffset"

#define maxTextScale2 0.3

@interface SPPageMenuLine2 : UIImageView
@property (nonatomic, copy) void(^hideBlock)(void);

@end

@implementation SPPageMenuLine2
...
@interface SPItem2 : UIButton

@property (nonatomic, assign) CGFloat imageRatio;
@property (nonatomic, assign) SPItemImagePosition2 imagePosition;
@end

@implementation SPItem2
...
- (void)initialize {
    _imageRatio = 0.5;
    _imagePosition = SPItemImagePosition2Default;
}
...
- (CGRect)imageRectForContentRect:(CGRect)contentRect {
    ...
    switch (self.imagePosition) {
        case SPItemImagePosition2Default:
        case SPItemImagePosition2Left: {
        ...
        case SPItemImagePosition2Top: {
            ...
            break;
        case SPItemImagePosition2Right: {
            _imageRatio = _imageRatio == 0.0 ? 0.5 : _imageRatio;
            CGFloat imageW =  contentRect.size.width * _imageRatio;
            CGFloat imageH = contentRect.size.height;
            ...
            break;
        case SPItemImagePosition2Bottom: {
            _imageRatio = _imageRatio == 0.0 ? 2.0/3.0 : _imageRatio;
            CGFloat imageW =  contentRect.size.width;
            CGFloat imageH = contentRect.size.height * _imageRatio;
...
- (void)setImagePosition:(SPItemImagePosition2)imagePosition {
...
@interface SPPageMenu2()
@property (nonatomic, assign) SPPageMenuTrackerStyle2 trackerStyle;
...
@property (nonatomic, weak) SPItem2 *functionButton;
@property (nonatomic, weak) CALayer *shadowLine;
@property (nonatomic, strong) NSMutableArray *buttons;
@property (nonatomic, strong) SPItem2 *selectedButton;
...
@implementation SPPageMenu2


#pragma mark - public

+ (instancetype)pageMenuWithFrame:(CGRect)frame trackerStyle:(SPPageMenuTrackerStyle2)trackerStyle {
    SPPageMenu2 *pageMenu = [[SPPageMenu2 alloc] initWithFrame:frame trackerStyle:trackerStyle];
    return pageMenu;
}

- (instancetype)initWithFrame:(CGRect)frame trackerStyle:(SPPageMenuTrackerStyle2)trackerStyle {
    if (self = [super init]) {
        self.frame = frame;
...
- (void)setItems:(NSArray *)items selectedItemIndex:(NSUInteger)selectedItemIndex {
    ...
    if (self.buttons.count) {
        ...
        SPItem2 *selectedButton = [self.buttons objectAtIndex:selectedItemIndex];
        [self buttonInPageMenuClicked:selectedButton];
        
        if (self.trackerStyle == SPPageMenuTrackerStyle2TextZoom) {
            [selectedButton setTitleColor:_selectedItemTitleColor forState:UIControlStateNormal];
            selectedButton.transform = CGAffineTransformMakeScale(1+maxTextScale2, 1+maxTextScale2);
        }
...
- (void)removeItemAtIndex:(NSUInteger)itemIndex animated:(BOOL)animated {
    if (itemIndex > self.items.count) {return;}
    // 被删除的按钮之后的按钮需要修改tag值
    for (SPItem2 *button in self.buttons) {
        if (button.tag-tagBaseValue2 > itemIndex) {
    ...
    if (itemIndex < self.buttons.count) {
        SPItem2 *button = [self.buttons objectAtIndex:itemIndex];
...
- (void)setTitle:(NSString *)title image:(UIImage *)image imagePosition:(SPItemImagePosition2)imagePosition imageRatio:(CGFloat)ratio forItemIndex:(NSUInteger)itemIndex {
    if (itemIndex < self.buttons.count) {
        SPItem2 *button = [self.buttons objectAtIndex:itemIndex];
        [button setTitle:title forState:UIControlStateNormal];
        [button setImage:image forState:UIControlStateNormal];
...
- (void)setFunctionButtonTitle:(NSString *)title image:(UIImage *)image imagePosition:(SPItemImagePosition2)imagePosition imageRatio:(CGFloat)ratio forState:(UIControlState)state {
...
- (void)setWidth:(CGFloat)width forItemAtIndex:(NSUInteger)itemIndex {
    [self.setupWidths setValue:@(width) forKey:[NSString stringWithFormat:@"%zd",itemIndex]];
}
...
- (void)addButton:(NSInteger)index object:(id)object animated:(BOOL)animated {
    ...
    for (SPItem2 *button in self.buttons) {
        if (button.tag-tagBaseValue2 >= index) {
            button.tag = button.tag + 1;
        }
    }
    SPItem2 *button = [SPItem2 buttonWithType:UIButtonTypeCustom];
    ...
    button.tag = tagBaseValue2 + index;
...
- (void)initialize {
    ...
    SPPageMenuLine2 *dividingLine = [[SPPageMenuLine2 alloc] init];
//    dividingLine.backgroundColor = [UIColor grayColor];
    ...
    SPItem2 *functionButton = [SPItem2 buttonWithType:UIButtonTypeCustom];
    ...
    shadowLine.shadowColor = H9COLOR.CGColor;
...
// 按钮点击方法
- (void)buttonInPageMenuClicked:(SPItem2 *)sender {
    if ([@[@"融资",@"国外",@"并购",@"投资机会"] containsObject:self.selectedButton.currentTitle] && self.selectedButton != sender) {
        
        [self setTitle:self.selectedButton.currentTitle image:nil imagePosition:SPItemImagePosition2Right imageRatio:0.001 forItemIndex:_selectedItemIndex];
        
        CGFloat w = self.selectedButton.currentTitle.length * 16+2;
    ...
    self.selectedButton.titleLabel.font = [UIFont systemFontOfSize:self.self.selectedButton.titleLabel.font.pointSize weight:UIFontWeightRegular];
    CGFloat fromIndex = self.selectedButton ? self.selectedButton.tag-tagBaseValue2 : sender.tag - tagBaseValue2;
    CGFloat toIndex = sender.tag - tagBaseValue2;
    ...
    if (self.trackerStyle == SPPageMenuTrackerStyleTextZoom) {
        ...
        sender.transform = CGAffineTransformMakeScale(1+maxTextScale2, 1+maxTextScale2);
    } else {
    ...
    if (@available(iOS 8.2, *)) {
        self.selectedButton.titleLabel.font = [UIFont systemFontOfSize:self.itemTitleFont.pointSize weight:UIFontWeightMedium];
    }else{
        self.selectedButton.titleLabel.font = [UIFont systemFontOfSize:self.itemTitleFont.pointSize];
    }
...
//
    if (fromIndex == toIndex &&
        [self.delegate respondsToSelector:@selector(pageMenu:currentButtonClickAtIndex:)]) {
        [self.delegate pageMenu:self currentButtonClickAtIndex:toIndex];
    }
}

- (void)buttonInPageMenuClicked222:(SPItem2 *)sender {
    CGFloat fromIndex = self.selectedButton ? self.selectedButton.tag-tagBaseValue2 : sender.tag - tagBaseValue2;
    CGFloat toIndex = sender.tag - tagBaseValue2;
    
    if ([@[@"融资",@"国外",@"并购",@"投资机会"] containsObject:self.selectedButton.currentTitle] && self.selectedButton != sender) {
        
        [self setTitle:self.selectedButton.currentTitle image:nil imagePosition:SPItemImagePosition2Right imageRatio:0.001 forItemIndex:fromIndex];
        
        CGFloat w = self.selectedButton.currentTitle.length * 16+2;
        [self setWidth:w forItemAtIndex:fromIndex];
        [self setNeedsLayout];
    }
...
- (void)moveItemScrollViewWithSelectedButton:(SPItem2 *)selectedButton {
...
// 功能按钮的点击方法
- (void)functionButtonClicked:(SPItem2 *)sender {
...
//        newFrame.size.width = fromButton.frame.size.width + wDistance * progress;   //wss 切换不改变宽度
        ...
        // 原先的x值
        CGFloat originX = fromButton.frame.origin.x+(fromButton.frame.size.width-9)*0.5;
        // 原先的宽度
        CGFloat originW = 9;//fromButton.titleLabel.font.pointSize;
        if (currentOffsetX - _beginOffsetX >= 0) { // 向左拖拽了
            if (progress < 0.5) {
...
- (void)zoomForTitleWithProgress:(CGFloat)progress fromButton:(UIButton *)fromButton toButton:(UIButton *)toButton {
    fromButton.transform = CGAffineTransformMakeScale((1 - progress) * maxTextScale2 + 1, (1 - progress) * maxTextScale2 + 1);
    toButton.transform = CGAffineTransformMakeScale(progress * maxTextScale2 + 1, progress * maxTextScale2 + 1);
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if (object == self.bridgeScrollView) {
        if ([keyPath isEqualToString:scrollViewContentOffset2]) {
...
- (void)setBridgeScrollView:(UIScrollView *)bridgeScrollView {
    ...
    if (bridgeScrollView) {
        ...
        [bridgeScrollView addObserver:self forKeyPath:scrollViewContentOffset2 options:NSKeyValueObservingOptionNew context:nil];
    } else {
        NSLog(@"你传了一个空的scrollView");
    }
}

- (void)setTrackerStyle:(SPPageMenuTrackerStyle2)trackerStyle {
    _trackerStyle = trackerStyle;
    switch (trackerStyle) {
        case SPPageMenuTrackerStyle2Line:
        case SPPageMenuTrackerStyle2LineLongerThanItem:
        case SPPageMenuTrackerStyle2LineAttachment:
            self.tracker.backgroundColor = _selectedItemTitleColor;
            break;
        case SPPageMenuTrackerStyle2RoundedRect:
        case SPPageMenuTrackerStyle2Rect:
...
- (void)setSelectedItemIndex:(NSUInteger)selectedItemIndex {
    _selectedItemIndex = selectedItemIndex;
    if (selectedItemIndex < self.buttons.count) {
        SPItem2 *button = [self.buttons objectAtIndex:selectedItemIndex];
        [self buttonInPageMenuClicked222:button];
    }
}

- (void)setDelegate:(id<SPPageMenuDelegate2>)delegate {
    if (delegate == _delegate) {return;}
    _delegate = delegate;
    if (self.buttons.count) {
        SPItem2 *button = [self.buttons objectAtIndex:_selectedItemIndex];
        [self delegatePerformMethodWithFromIndex:button.tag-tagBaseValue2 toIndex:button.tag-tagBaseValue2];
...
- (void)setPermutationWay:(SPPageMenuPermutationWay2)permutationWay {
...
- (void)layoutSubviews {
    ...
    for (int i= 0 ; i < self.buttons.count; i++) {
        ...
        UIFont *font;
        if (@available(iOS 8.2, *)) {
            font = [UIFont systemFontOfSize:_itemTitleFont.pointSize weight:UIFontWeightMedium];
        }else{
            font = [UIFont systemFontOfSize:_itemTitleFont.pointSize];
        }
        CGFloat setupButtonW = [[self.setupWidths objectForKey:[NSString stringWithFormat:@"%zd",i]] floatValue];
        CGFloat textW = [button.titleLabel.text boundingRectWithSize:CGSizeMake(MAXFLOAT, itemScrollViewH) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:font} context:nil].size.width;
        ...
        if (button.currentTitle && !button.currentImage) {
            ...
            contentW = imageW;
        } else if (button.currentTitle && button.currentImage && (button.imagePosition == SPItemImagePosition2Right || button.imagePosition == SPItemImagePosition2Left)) {
            contentW = textW + imageW;
        } else if (button.currentTitle && button.currentImage && (button.imagePosition == SPItemImagePosition2Top || button.imagePosition == SPItemImagePosition2Bottom)) {
    ...
    [self.buttons enumerateObjectsUsingBlock:^(SPItem2 *button, NSUInteger idx, BOOL * _Nonnull stop) {
        CGFloat setupButtonW = [[self.setupWidths objectForKey:[NSString stringWithFormat:@"%zd",idx]] floatValue];
        if (self.permutationWay == SPPageMenuPermutationWay2ScrollAdaptContent) {
        ...
        } else if (self.permutationWay == SPPageMenuPermutationWay2NotScrollEqualWidths) {
            // 求出外界设置的按钮宽度之和
...
- (void)resetSetupTrackerFrameWithSelectedButton:(UIButton *)selectedButton {
    ...
    switch (self.trackerStyle) {
        ...
        {
            trackerW = 9;  //关注模块
//            if ([selectedButton.titleLabel.text containsString:@"项目"] || [selectedButton.titleLabel.text containsString:@"机构"]) {
//                trackerW = 30;  //关注模块
//            }else{
//                trackerW = self.tracker.frame.size.width > 0 ? self.tracker.frame.size.width:selectedButtonWidth;  //wss 切换不改变宽度
//            }
            trackerW = 20;//selectedButtonWidth;
            trackerH = _trackerHeight;
            trackerX = selectedButton.frame.origin.x;
            trackerY = self.itemScrollView.bounds.size.height - trackerH-0;
        ...
        case SPPageMenuTrackerStyle2LineAttachment:
        {
            trackerW = selectedButtonWidth ? selectedButtonWidth : 0; // 固定宽度为字体大小
            trackerW = 9;
            trackerH = _trackerHeight;
            trackerX = selectedButton.frame.origin.x;
            trackerY = self.itemScrollView.bounds.size.height - trackerH - 6;
            if (self.isAttention) {
                trackerY += 2;
            }
            self.tracker.frame = CGRectMake(trackerX, trackerY, trackerW, trackerH);
...
- (void)dealloc {
    [self.bridgeScrollView removeObserver:self forKeyPath:scrollViewContentOffset2];
}
