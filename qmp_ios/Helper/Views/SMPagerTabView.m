...
- (void)buildUI {
    ...
    for (int i = 0; i < number; i++) {
        ...
        [itemButton setTitleColor:RGB(202, 68, 61, 1) forState:UIControlStateSelected];
        ...
        aRedDot.backgroundColor = RED_TEXTCOLOR;
...
- (void)layoutSubviews {
    if (_isBuildUI) {
        ...
        for (int i = 0; i < [self.viewsArray count]; i++) {
            ...
            vc.view.frame = CGRectMake(self.bodyScrollView.width * i, self.tabFrameHeight, self.bodyScrollView.width, self.bodyScrollView.height-40);
        }
    }
}
...
- (UIColor *)tabButtonTitleColorForSelected {
    if (!_tabButtonTitleColorForSelected) {
        self.tabButtonTitleColorForSelected = RED_TEXTCOLOR;//[UIColor redColor];
    }
...
- (UIView *)selectedLine {
    if (!_selectedLine) {
        ...
        UIView *lineV = [[UIView alloc] initWithFrame:CGRectMake(0, self.tabView.height - 2, SCREENW, 1)];
        lineV.backgroundColor = RGBLineGray;
        [self addSubview:lineV];
        
        self.selectedLine = [[UIView alloc] initWithFrame:CGRectMake(0, self.tabView.height - 2, self.selectedLineWidth, 1)];
        _selectedLine.backgroundColor = RGB(202, 68, 61, 0.8);//;RGB(202, 68, 61, 1)
