...
- (void)setShowRefreshHeader:(BOOL)showRefreshHeader
{
    if (_showRefreshHeader != showRefreshHeader) {
        ...
        if (_showRefreshHeader) {
            ...
            MJRefreshNormalHeader *_mjHeader = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
                [weakSelf tableViewDidTriggerHeaderRefresh];
            }];
            _mjHeader.stateLabel.font = [UIFont systemFontOfSize:14];
            _mjHeader.stateLabel.textColor = H9COLOR;
            [_mjHeader setTitle:@"更新中" forState:MJRefreshStateRefreshing];
            
            //        _mjHeader.labelLeftInset = 50;
            _mjHeader.lastUpdatedTimeLabel.hidden = YES;
            self.tableView.mj_header = _mjHeader;
