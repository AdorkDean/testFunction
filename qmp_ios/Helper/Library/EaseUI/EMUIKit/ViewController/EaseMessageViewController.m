...
- (void)viewDidLoad {
    ...
    if ([self.chatFriendM.usercode isEqualToString:QMPHelperUserCode]) {
        self.chatFriendM.headimgurl = kHeadImgUrl;
        self.chatFriendM.nickname = kCustomerName;
    }
    ...
    if (isiPhoneX) {
        self.chatToolbar = [[EaseChatToolbar alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - chatbarHeight-KSafeHeightBottom, self.view.frame.size.width, chatbarHeight) type:barType];
    ...
    [[EaseBaseMessageCell appearance] setSendBubbleBackgroundImage:[[UIImage imageNamed:@"EaseUIResource.bundle/chat_sender_bg2"] stretchableImageWithLeftCapWidth:5 topCapHeight:35]];
    [[EaseBaseMessageCell appearance] setRecvBubbleBackgroundImage:[[UIImage imageNamed:@"EaseUIResource.bundle/chat_receiver_bg2"] stretchableImageWithLeftCapWidth:35 topCapHeight:35]];
...
//    [[EaseBaseMessageCell appearance] setAvatarSize:45.f];
//    [[EaseBaseMessageCell appearance] setAvatarCornerRadius:22.5f];
    
    [[EaseChatBarMoreView appearance] setMoreViewBackgroundColor:[UIColor colorWithRed:240 / 255.0 green:242 / 255.0 blue:247 / 255.0 alpha:1.0]];
    
    if (isiPhoneX) {
        self.tableView.contentInset = UIEdgeInsetsMake(0, 0, KSafeHeightBottom, 0);
    }
...
- (void)saveChatroom:(EMChatroom *)chatroom
{
    ...
    {
        [chatRooms setValue:chatroomName forKey:chatroom.chatroomId];
        [ud setValue:chatRooms forKey:key];
        [ud synchronize];
    }
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section{
    return self.showSectionView ? 50:0.1;
}
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if (self.showSectionView) {
        UIView *headerV = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 50)];
        UILabel *lab = [[UILabel alloc]initWithFrame:headerV.bounds];
        [lab labelWithFontSize:13 textColor:H5COLOR];
        lab.textAlignment = NSTextAlignmentCenter;
        lab.text = @"你们已经是好友了，打声招呼吧";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        if([object isKindOfClass:[EaseMessageModel class]]){
            EaseMessageModel *message = object;
            if ([message.text isEqualToString:MESSAGE_FRIENDALERT] || [message.text isEqualToString:MESSAGE_BEFRIEND] || [message.text isEqualToString:MESSAGE_REJECTED] || [message.text isEqualToString:MESSAGE_REJECTRECEIVE]) {
                NSString *TimeCellIdentifier = [EaseMessageTimeCell cellIdentifier];
                EaseMessageTimeCell *timeCell = (EaseMessageTimeCell *)[tableView dequeueReusableCellWithIdentifier:TimeCellIdentifier];
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(isEmotionMessageFormessageViewController:messageModel:)]) {
            ...
            if (flag) {
                ...
                if (_dataSource && [_dataSource respondsToSelector:@selector(emotionURLFormessageViewController:messageModel:)]) {
                    ...
                    if (emotion) {
                        model.image = [UIImage sd_animatedGIFWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:emotion.emotionOriginal]]];
...
//        sendCell.sendBubbleBackgroundImage = [UIImage imageNamed:@"sendBubbleBackgroundImage"];
//        sendCell.recvBubbleBackgroundImage = [UIImage imageNamed:@"recvBubbleBackgroundImage"];
        sendCell.messageTextColor = H5COLOR;
        
        sendCell.messageNameIsHidden = YES;
        sendCell.messageTextFont = [UIFont systemFontOfSize:15];
//        sendCell.avatarSize = 50;
        sendCell.model = model;
        
        sendCell.avatarView.userInteractionEnabled = YES;
        if(sendCell.model.isSender){
//            sendCell.avatarView.userInteractionEnabled = NO;
            
            UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapHeaderImgView2)];
            [sendCell.avatarView addGestureRecognizer:tap];
        }else{
...
//头像进个人主页
- (void)tapHeaderImgView:(UITapGestureRecognizer*)tap{
    if ([self.conversation.conversationId isEqualToString:QMPHelperUserCode]) {
        return;
    }
...
- (void)chatToolbarDidChangeFrameToHeight:(CGFloat)toHeight{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        rect.origin.y = 0;
        rect.size.height = self.view.frame.size.height - toHeight - 45;
...
- (void)didStartRecordingVoiceAction:(UIView *)recordView
{
    ...
    [self _canRecordCompletion:^(EMRecordResponse recordResponse) {
        switch (recordResponse) {
            ...
            case EMCanNotRecord:
            {
                UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"prompt", @"Prompt") message:NSLocalizedString(@"setting.microphoneNoAuthority", @"setting.microphoneAuthority") delegate:nil cancelButtonTitle:NSLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
                [alertView show];
            }
                break;
            default:
...
- (NSArray *)formatMessages:(NSArray *)messages
{
    ...
    for (EMMessage *message in messages) {
        ...
        CGFloat interval = (self.messageTimeIntervalTag - message.timestamp) / 1000/60;
        if (self.messageTimeIntervalTag < 0 || interval > 10 || interval < -10) {
 ...
 * 提示只能发生3条消息
 */
- (void)restrictionsDeal {
    
    EMMessage *message = [EaseSDKHelper getTextMessage:MESSAGE_FRIENDALERT to:self.conversation.conversationId messageType:EMChatTypeChat messageExt:@{@"userAvatar":[WechatUserInfo shared].headimgurl,@"userNick":[WechatUserInfo shared].nickname}];
...
- (void)appendMessage:(NSString*)messageText{
    
    EMMessage *message = [EaseSDKHelper getTextMessage:messageText to:self.conversation.conversationId messageType:EMChatTypeChat messageExt:@{@"userAvatar":[WechatUserInfo shared].headimgurl,@"userNick":[WechatUserInfo shared].nickname, @"otherAvatar":self.chatFriendM.headimgurl, @"otherNick":self.chatFriendM.nickname}];
...
{
    //非好友 三条消息限制 不算客服 和 客服本人
//    if (self.chatFriendM.is_friend.integerValue == 0  && ![self.chatFriendM.usercode isEqualToString:QMPHelperUserCode] && ![[WechatUserInfo shared].usercode isEqualToString:QMPHelperUserCode]) {
//        NSInteger count = 0;
...
//                    EaseMessageModel *messageM = (EaseMessageModel*)model;
//                    if ([messageM.message.from isEqualToString:[WechatUserInfo shared].usercode]) { //自己发的
//                        if (count >= 5) {
//                            [self restrictionsDeal];
//                            return;
//
//                        }else if(![messageM.text isEqualToString:MESSAGE_FRIENDALERT]){
//                            count++;
//                        }
//                    }else{
//                        if (count >= 5) {
...
//                }
//            }
//        }else if(self.dataArray.count == 5){ //第一次聊天
//            NSInteger count = 0;
...
//
    ...
    //头像昵称
    NSDictionary *dic;
    if ([self.conversation.ext[@"from"] isEqualToString:@"investor"]) {
        dic = @{@"userAvatar":[WechatUserInfo shared].headimgurl,@"userNick":[WechatUserInfo shared].nickname,@"fromInvestor":@"1", @"otherAvatar":self.chatFriendM.headimgurl, @"otherNick":self.chatFriendM.nickname};

    }else{
        dic = @{@"userAvatar":[WechatUserInfo shared].headimgurl,@"userNick":[WechatUserInfo shared].nickname, @"otherAvatar":self.chatFriendM.headimgurl, @"otherNick":self.chatFriendM.nickname};
    }
    if (!message.ext) {
        message.ext = dic;
    }
...
//    } else {
    ...
    [QMPEvent event:@"chatpage_sendmsg_click"];
}
