...
CGFloat const EaseMessageCellPaddingTB = 7.5;
CGFloat const EaseMessageCellPaddingLR = 16;

NSString *const EaseMessageCellIdentifierRecvText = @"EaseMessageCellRecvText";
NSString *const EaseMessageCellIdentifierRecvShare = @"EaseMessageCellRecvShare";
...
NSString *const EaseMessageCellIdentifierSendShare = @"EaseMessageCellSendShare";
...
@interface EaseMessageCell() <UIGestureRecognizerDelegate>
{
...
+ (void)initialize
{
    ...
    cell.messageTextColor = HTColorFromRGB(0x33333);
...
//    _hasRead.hidden = YES;
    _hasRead.clipsToBounds = YES;
    _hasRead.layer.cornerRadius = 3.5;
    _hasRead.backgroundColor = [UIColor redColor];
    [_hasRead sizeToFit];
...
{
    //bubble view
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.bubbleView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeBottom multiplier:1.0 constant:-EaseMessageCellPaddingTB]];
...
//    self.bubbleMaxWidthConstraint.active = YES;
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.hasRead attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeTop multiplier:1.0 constant:7.5]];
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.hasRead attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:7]];
//    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.hasRead attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:7]];
...
- (void)_updateHasReadWidthConstraint
{
    if (_hasRead) {
        ...
        self.hasReadWidthConstraint = [NSLayoutConstraint constraintWithItem:_hasRead attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:0 constant:7];
...
- (void)setModel:(id<IMessageModel>)model
{
    _model = model;
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        [self setCustomModel:model]; //gif
    } else {
        switch (model.bodyType) {
            case EMMessageBodyTypeText:
            {
                NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithAttributedString:[[EaseEmotionEscape sharedInstance] attStringFromTextForChatting:model.text textFont:self.messageTextFont]];
                
                if (model.isSender) {
                    [str setAttributes:@{NSForegroundColorAttributeName:[UIColor whiteColor]} range:NSMakeRange(0, str.length)];
                } else {
                    [str setAttributes:@{NSForegroundColorAttributeName:_messageTextColor} range:NSMakeRange(0, str.length)];
                }
...
//                            NSLog(@"%@",[model.text substringWithRange:result.range]);
...
//                            NSLog(@"%@",[model.text substringWithRange:result.range]);
//                            [ranges addObject:[NSValue valueWithRange:result.range]];
//                            [str setAttributes:@{NSFontAttributeName: self.messageTextFont, NSForegroundColorAttributeName:BLUE_TITLE_COLOR} range:result.range];
//                        }
//                    }
//                }];                
                ...
                _bubbleView.textLabel.attributedText = str;
...
+ (CGFloat)cellHeightWithModel:(id<IMessageModel>)model
{
    ...
    CGFloat height = EaseMessageCellPaddingTB + cell.bubbleMargin.top + cell.bubbleMargin.bottom;
    ...
    switch (model.bodyType) {
        case EMMessageBodyTypeText:
        {
            ...
            height += (rect.size.height > 20 ? rect.size.height + 5 : 20);
        }
            break;
        case EMMessageBodyTypeImage:
        case EMMessageBodyTypeVideo:
    ...
    height += EaseMessageCellPaddingTB;
