...
- (void)drawBar:(NSMutableArray *)valueArray{
    ...
    if ([subObject isKindOfClass:[NSString class]]) {
        for (NSInteger i = 0; i < valueArray.count; i++) {
            ...
            CGFloat xPos = self.horizontalAxis.axisStartXPos + self.horizontalAxis.yAxisLine.yLineWidth + 8;
            ...
            ZFHorizontalBar * bar = [[ZFHorizontalBar alloc] initWithFrame:CGRectMake(xPos, yPos, width-45, height)];
            bar.groupIndex = 0;
            bar.barIndex = i;
        ...
        if ([[subObject firstObject] isKindOfClass:[NSString class]]) {
            //bar总数
            NSInteger count = [valueArray count] * [(NSArray*)subObject count];
            for (NSInteger i = 0; i < count; i++) {
                //每组bar的下标
                NSInteger barIndex = i % [(NSArray*)subObject count];
                //组下标
                NSInteger groupIndex = i / [(NSArray*)subObject count];
...
- (void)setValueLabelOnChart:(NSMutableArray *)valueArray{
    ...
    if ([subObject isKindOfClass:[NSString class]]) {
        for (NSInteger i = 0; i < self.barArray.count; i++) {
            ...
            //label的中心点
            CGPoint label_center = CGPointMake(bar.endXPos + self.horizontalAxis.yAxisLine.yLineStartXPos + (rect.size.width + 10) * 0.5 + _valueLabelToBarPadding+6, bar.center.y);
