...
- (void)commonInit{
    ...
    _valueCenterToCircleCenterPadding = 20.f;
    _isShadow = YES;
...
- (void)drawGenericChart{
    ...
    self.genericAxis.scrollEnabled = YES;
    if (iPhone5) {
        self.genericAxis.scrollEnabled = YES;
...
- (void)setYLineReverse:(BOOL)yLineReverse {
    _yLineReverse = yLineReverse;
    self.genericAxis.yLineReverse = self.yLineReverse;
}
#pragma mark - 画圆
...
- (void)drawCircle{
    ...
    if ([subObject isKindOfClass:[NSString class]]) {
        for (NSInteger i = 0; i < self.genericAxis.xLineValueArray.count; i++) {
            ...
            if (self.yLineReverse) {
                percent = (self.genericAxis.yLineMaxValue - [self.genericAxis.xLineValueArray[i] floatValue]) / (self.genericAxis.yLineMaxValue - self.genericAxis.yLineMinValue);
            }
        ...
        if ([[subObject firstObject] isKindOfClass:[NSString class]]) {
            ...
            for (NSInteger lineIndex = 0; lineIndex < self.genericAxis.xLineValueArray.count; lineIndex++) {
                ...
                //第几个圆
                for (NSInteger circleIndex = 0; circleIndex < [(NSArray*)subObject count]; circleIndex++) {
                    BOOL isOverrun = NO;//记录是否超出上限
...
- (void)addValueLabel:(NSMutableArray *)circleArray index:(NSInteger)index colorIndex:(NSInteger)colorIndex valueArray:(NSMutableArray *)valueArray valuePosition:(kChartValuePosition)valuePosition{
    ...
    popoverLabel.textColor = COLOR2D343A;//_colorArray[colorIndex];
...
- (void)circleAction:(ZFCircle *)sender{
    if ([self.delegate respondsToSelector:@selector(lineChart:didSelectCircleAtLineIndex:circleIndex:circle:popoverLabel:)]) {
        ...
        if ([subObject isKindOfClass:[NSString class]]) {
            ...
            popoverLabel = self.popoverLaberArray[sender.lineIndex * [(NSArray*)subObject count] + sender.circleIndex];
        }
