...
//            CGFloat laWidth = [PublicTool widthOfString:self.yLineNameArray[i] height:CGFLOAT_MAX fontSize:_yLineNameFont.pointSize];
            ZFLabel * label = [[ZFLabel alloc] initWithFrame:CGRectMake(0, 0, width, height + _groupPadding * 0.5)];
            ...
            label.contentMode = UIViewContentModeRight;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    //顶部遮罩层
    self.maskView.frame = CGRectMake(self.maskView.frame.origin.x, scrollView.contentOffset.y, self.maskView.frame.size.width, 30);
...
- (void)setFrame:(CGRect)frame{
    ...
    self.maskView.frame = CGRectMake(0, 0, self.frame.size.width, 30);
}
...
- (CGFloat)axisStartXPos{
    CGFloat maxX = 0;
    for (NSString *name in self.yLineNameArray) {
        CGFloat laWidth = [name boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:_yLineNameFont.pointSize]} context:nil].size.width;
        if (maxX < laWidth) {
            maxX = laWidth;
        }
    }
    ...
    self.yAxisLine.yLineStartXPos = maxX;    
