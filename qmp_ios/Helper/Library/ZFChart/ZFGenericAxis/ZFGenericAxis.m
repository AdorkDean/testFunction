...
- (void)addUnitLabel{
    if (_unit) {
        ...
        CGFloat center_xPos = self.yAxisLine.yLineStartXPos + width/2.0 + 6;//self.yAxisLine.yLineStartXPos / 2;
        CGFloat center_yPos = self.yAxisLine.yLineEndYPos-6;
        ...
        self.unitLabel.font = [UIFont systemFontOfSize:10.f];
...
- (void)setXLineNameLabel{
    if (self.xLineNameArray.count > 0) {
        for (NSInteger i = 0; i < self.xLineNameArray.count; i++) {
            ...
            ZFLabel * label = [[ZFLabel alloc] initWithFrame:CGRectMake(0, 0, rect.size.width+8, rect.size.height)];
    ...
    for (NSInteger i = 0; i <= _yLineSectionCount; i++) {
        ...
        if (_valueType == kValueTypeInteger) {
            if (self.yLineReverse) {
                label.left = -2;
                label.text = [NSString stringWithFormat:@"%.0f", _yLineMaxValue - valueAverage * i];
            } else {
                label.left = 0;
                label.text = [NSString stringWithFormat:@"%.0f", _yLineMinValue + valueAverage * i];
            }
...
- (UIBezierPath *)drawYAxisLineSection:(NSInteger)i sectionLength:(CGFloat)sectionLength{
    ...
    CGFloat yStartPos = self.yAxisLine.yLineStartYPos - (self.yAxisLine.yLineHeight - ZFAxisLineGapFromAxisLineMaxValueToArrow) / _yLineSectionCount * (i + 1);
    [bezier moveToPoint:CGPointMake(self.yAxisLine.yLineStartXPos, yStartPos)];
    [bezier addLineToPoint:CGPointMake(self.yAxisLine.yLineStartXPos + sectionLength-20, yStartPos)];
    return bezier;
}
