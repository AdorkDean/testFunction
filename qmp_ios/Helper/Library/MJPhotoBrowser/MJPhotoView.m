...
- (void)photoStartLoad
{
    if (_photo.image) {
        ...
        __weak typeof(self)  photoView = self;
        [photoView photoDidFinishLoadWithImage:_photo.image];
...
#warning --12.14 发现崩溃loading.progress
        __weak typeof(self) photoView = self;
        __weak typeof(_photoLoadingView) loading = _photoLoadingView;
//        __unsafe_unretained MJPhotoView *photoView = self;
...
- (void)adjustFrame
{
    ...
    if (_photo.firstShow) { // 第一次显示的图片
        ...
        _imageView.frame = [_photo.srcView convertRect:_photo.srcView.bounds toView:KEYWindow];
