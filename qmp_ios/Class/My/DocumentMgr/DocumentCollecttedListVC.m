...
@interface DocumentCollecttedListVC ()<UITableViewDelegate,UITableViewDataSource,DownloadViewDelegate,OpenDocumentDelegate,UISearchBarDelegate,UIScrollViewDelegate,changeNoPdfCollectionStatusDelegate>
{
...
@implementation DocumentCollecttedListVC
...
- (void)viewDidLoad {
    ...
    [self showHUD];
    [self getCollecttedList:self.currentPage ofNum:self.numPerPage];
}
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar{
    ...
    if (![searchBar.text isEqualToString:@""]) {
        _searchCurrentPage = 1;
        _isSearch = YES;
        [self getCollecttedList:_searchCurrentPage ofNum:_searchNum];
...
- (void)changeNoPdfCollectionStatusByClick:(ReportModel *)changeModel{
    if ([changeModel.collectFlag isEqualToString:@"0"]) {
        if (self.isSearch) {
            [self.searchArr removeObjectAtIndex:_selectedIndexPath.row];
        }else{
            [self.tableData removeObjectAtIndex:_selectedIndexPath.row];
        }
        [self.tableView deleteRowsAtIndexPaths:@[_selectedIndexPath] withRowAnimation:UITableViewRowAnimationNone];
    }
}
...
- (void)getCollecttedList:(NSInteger)currentPage ofNum:(NSInteger)num{
    
    self.info = @"";
    
    if([TestNetWorkReached networkIsReachedNoAlert]){
        
        NSDictionary *param = @{@"page":[NSString stringWithFormat:@"%ld",(long)self.currentPage],@"num":[NSString stringWithFormat:@"%ld",(long)self.numPerPage],@"keyword":[PublicTool isNull:_mySearchBar.text] ? @"":_mySearchBar.text, @"type":@"1"};
