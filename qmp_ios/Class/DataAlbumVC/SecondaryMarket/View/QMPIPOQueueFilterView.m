...
#import "QMPIPOQueueFilterView.h"
#import "FinanceEventFilterView.h"
#import "RzPartfilterTableViewCell.h"
#import "RzfilterTableViewCell.h"

@interface QMPIPOQueueFilterView () <UITableViewDataSource, UITableViewDelegate>
...
@end
@implementation QMPIPOQueueFilterView
- (instancetype)init {
    self = [super init];
    if (self) {
...
- (void)show {
    ...
    self.oldSelectedTagData2 = [NSArray arrayWithArray:[self filterBoard]];
    self.oldSelectedRoundData2 = [NSArray arrayWithArray:[self filterState]];
...
- (BOOL)filterChange {
    BOOL a = [self aa:[self filterBoard] bb:self.oldSelectedTagData2];
    BOOL b = [self aa:[self filterState] bb:self.oldSelectedRoundData2];
...
- (NSArray *)allStateData {
    if (!_allStateData) {
        _allStateData = @[
                          @{@"name":@"上会通过", @"selected":@"0"},
                          @{@"name":@"中止审查", @"selected":@"0"},
                          @{@"name":@"取消审核", @"selected":@"0"},
                          @{@"name":@"已反馈", @"selected":@"0"},
                          @{@"name":@"暂缓表决", @"selected":@"0"},
                          @{@"name":@"暂缓发行", @"selected":@"0"},
                          @{@"name":@"IPO被否", @"selected":@"0"},
                          @{@"name":@"预先披露", @"selected":@"0"},
                          @{@"name":@"预先披露更新", @"selected":@"0"},
                          ];
    }
...
- (void)initFilterSections {
    ...
    section2.title = @"审核状态";
    section2.type = @"state";
    section2.dataArray = [self fix:self.allStateData];
    ...
    section3.title = @"行业领域";
    section3.type = @"field";
    section3.dataArray = [self fix:self.allFieldData];
...
- (NSArray *)filterBoard {
    for (QMPFilterSection *section in self.sections) {
        if ([section.title isEqualToString:@"申请板块"]) {
...
- (NSArray *)filterState {
    for (QMPFilterSection *section in self.sections) {
        if ([section.title isEqualToString:@"审核状态"]) {
            return section.selectedArray;
        }
    }
    return nil;
