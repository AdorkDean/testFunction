...
- (void)scrollToIndex:(int)targetIndex
{
    ...
    if ([self.delegate respondsToSelector:@selector(bannerView:didShowAtIndex:)]) {
        [self.delegate bannerView:self didShowAtIndex:targetIndex % self.dataSource.count];  //indexPath.row % self.dataSource.count
    }
}
...
- (void)snapToPage {
    ...
    if (!CGPointEqualToPoint(pageOffset, currentOffset)) {
        ...
        [self.delegate bannerView:self didShowAtIndex:[self currentIndex] % self.dataSource.count];  //indexPath.row % self.dataSource.count
...
//        self.bannerLayout.spacing = (self.bannerLayout.maxWidth - self.bannerLayout.itemWidth) * 0.5 + 4;
        ...
        self.bannerLayout.spacing = iPad ? 70:40;
